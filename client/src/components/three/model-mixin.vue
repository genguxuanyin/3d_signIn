<template>
    <div ref="container" style="width: 100%; height: 100%; margin: 0; border: 0; padding: 0;"></div>
</template>
...
export default {
    ...
    mounted() {
        ...
        // this.webglRenderer = new WebGLRenderer( { antialias: true, alpha: true, canvas: this.$refs.canvas } )
        // this.webglRenderer.shadowMap.enabled = true;

        this.css3dRenderer = new CSS3DRenderer(this.$refs.canvas);
    ...
    methods: {
        ...
        updateRenderer() {
            ...
            // webglRenderer.setSize( this.size.width, this.size.height );
            // webglRenderer.setPixelRatio( window.devicePixelRatio || 1 );
            // webglRenderer.setClearColor( new Color( this.backgroundColor ).getHex() );
            // webglRenderer.setClearAlpha( this.backgroundAlpha );

            const css3dRenderer = this.css3dRenderer;
            css3dRenderer.setSize( this.size.width, this.size.height );
        ...
        updateCamera() {
            ...
            if ( !this.cameraLookAt && !this.cameraPosition && !this.cameraRotation && !this.cameraUp ) {
                ...
                this.cameraPosition && camera.position.set( this.cameraPosition.x, this.cameraPosition.y, this.cameraPosition.z )
                this.cameraRotation && camera.rotation.set( this.cameraRotation.x, this.cameraRotation.y, this.cameraRotation.z )
                this.cameraUp && camera.up.set( this.cameraUp.x, this.cameraUp.y, this.cameraUp.z )

                this.cameraLookAt && camera.lookAt( new Vector3( this.cameraLookAt.x, this.cameraLookAt.y, this.cameraLookAt.z ) )
